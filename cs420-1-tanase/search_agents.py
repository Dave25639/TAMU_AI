...
class CornersProblem(search.SearchProblem):
    ...
    def is_goal_state(self, state):
        ...
        """
        currPosition, cornersLeft = state
    ...
    def get_successors(self, state):
        curr_pos, corners_left = state
        successors = []
        """For each possible direction, check if it's a valid move and if so, add to successors."""
        for action in [
            Directions.NORTH,
            Directions.SOUTH,
            Directions.EAST,
            ...
            if self.walls[nextx][nexty] == False:
                updated_corners = corners_left
                if new_pos in corners_left:
                    temp = list(corners_left)
                    temp.remove(new_pos)
                    updated_corners = tuple(temp)
                next_state = ((nextx, nexty), updated_corners)
                cost = 1
...
def corners_heuristic(state, problem):
    ...
    """This heuristic works by finding the distance to nearest corner + distance to next closest corner and so on until it hits all remaining corners."""
    """This give an incentive to move towards a path that leads to you getting closer to passing through all 4 corners."""
    corners = problem.corners  # These are the corner coordinates
    walls = problem.walls  # These are the walls of the maze, as a Grid (game.py)
    prevPoint, cornersLeft = state
    ...
    for i in range(len(cornersLeft)):
        ...
        for corner in cornersLeft:
            ...
            if dist < shortest:
                shortest = dist
                remove = corner
        shortestDistances.append(shortest)
        cornersLeft.remove(remove)
        prevPoint = remove

    return sum(shortestDistances)   # Score: Manhattan: 692 Euclidean: 806

    """Previous strategies and their respective scores."""
    # distances = []
    # for cornerLeft in corners:
    #     xy1 = position
    #     xy2 = cornerLeft
    #     distances.append(((xy1[0] - xy2[0]) ** 2 + (xy1[1] - xy2[1]) ** 2) ** 0.5)
    #
    # return min(distances)   # Score: Manhattan: 1653 Euclidean: 1685
...
def food_heuristic(state, problem):
    ...
    """
    """This heuristic works by finding the furthest dot. I am not exactly sure how it makes the problem explore less nodes
    but I think it has something to do with staying in the center of mass of the dots and this makes it more efficient."""
    position, food_grid = state
    max_distance = 0
    for foodLocation in food_grid.as_list():
        max_distance = max(max_distance, maze_distance(position, foodLocation, problem.starting_game_state))

    return max_distance

    """Previous attempt that uses manhattan distance."""
    # position, food_grid = state
    #
    # shortestDistances = []
    # foodLeft = food_grid.as_list()
    # prevPoint = position
    # for i in range(len(foodLeft)):
    #     shortest = sys.maxsize
    #     remove = ()
    #     for foodPoint in foodLeft:
    #         xy1 = prevPoint
    #         xy2 = foodPoint
    #         #dist = ((xy1[0] - xy2[0]) ** 2 + (xy1[1] - xy2[1]) ** 2) ** 0.5
    #         dist = abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])
    ...
    #             remove = foodPoint
    #     shortestDistances.append(shortest)
    #     foodLeft.remove(remove)
    #     prevPoint = remove
    #
    # return sum(shortestDistances)/2
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def find_path_to_closest_dot(self, game_state):
        ...
        game_state.
        """
        """Pretty basic algorithm that just uses BFS to find next closest dot."""
